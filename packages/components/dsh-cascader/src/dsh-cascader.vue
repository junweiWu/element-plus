<template>
  ...
  >
    ...
    <template #content>
      ...
      <div v-if="showFoot" class="cascader-footer">
...
const COMPONENT_NAME = 'DshCascader'
...
// 临时存储值，用于取消按钮点击时恢复原值
const tempCheckedNodes: Ref<CascaderNode[]> = ref([])
...
const handleCancel = () => {
  ...
  if (length - 1 === tempCheckedNodes.value.length) {
    cascaderPanelRef.value
      ...
      ?.forEach((node: CascaderNode) => {
        node.doCheck(true)
        cascaderPanelRef.value?.expandParentNode(node)
      })
  } else {
    tempCheckedNodes.value.forEach((tempNode: CascaderNode) => {
      cascaderPanelRef.value
        ?.getFlattedNodes(true)
        // eslint-disable-next-line array-callback-return
        ?.some((node: CascaderNode) => {
...
:deep(.el-cascader-node:hover .select-tag) {
  display: block;
  background: #005bf5;
  color: #fff;
  position: relative;
...
:deep(.el-cascader-node:hover .select-tag::before) {
  content: '';
  width: 0;
  height: 0;
  position: absolute;
