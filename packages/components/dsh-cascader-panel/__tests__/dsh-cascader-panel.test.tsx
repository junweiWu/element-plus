...
const lazyLoad: DshLazyLoad = (node, resolve) => {
  const { level } = node
  setTimeout(() => {
...
describe('CascaderPanel.vue', () => {
  ...
  test('with default value', async () => {
    const value = ref<null | DshCascaderValue>(['zhejiang', 'hangzhou'])
    const wrapper = mount(() => (
  ...
  test('expand by hover', async () => {
    const props: DshCascaderProps = { expandTrigger: 'hover' }
    const wrapper = mount(() => (
  ...
  test('options async load with default value', async () => {
    const options = ref<DshCascaderOption[]>([])
    const wrapper = mount(() => (
      <CascaderPanel
